...
export function getRecommendations(charities_donated, charities_all) {
    ...
    const recommendations = charities_donated.map(
        (res) => {
            const index = corpus.getDocumentIdentifiers().findIndex((element) => element === res.charityName)
...
// testing
//const charities = charity_names.map((res) => JSON.parse(JSON.stringify({charityName: res})))
